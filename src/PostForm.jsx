...
export default function PostForm({
    ...
    const handleSubmit_updateTypeCheck = (form) => {
        const createNewMessage = (form, validImages, newNo, ReplyNo, targetCampId) => ({
            // 編集含めて全て必要
        ...
        if (formType === "diplomacy") {
            if (!confirm(`送信した外交文書は削除出来ません。送信してもよろしいですか？`)) return false;
        }
    ...
    return (
        <div
            className="basis-full max-h-[90vh]"
            onClick={(e) => e.stopPropagation()}
        >
            <form
                className="w-full flex-grow bg-white p-5 flex flex-col max-h-[90vh] overflow-y-auto"
                onSubmit={(e) => {
                    e.preventDefault();
            ...
            >
                ...
                <textarea
                    name="content"
                    placeholder="内容"
                    className="mb-2 p-2 border h-36"
                ...
                <div className="flex">
                    ...
                    <div className="ml-auto mt-auto">
                        <button
                            type="submit"
                            className={`text-xl font-bold m-2 px-8 py-4 border-none rounded ${isSubmitDisabled ? "bg-blue-800 text-slate-400" : "bg-blue-600 text-white"}`}
    ...
    return (
        <div className="flex items-center mb-2 self-start">
            ...
            <select
                ...
                value={selectedCamp}
            >
                {Object.entries(HAKONIWAData.campNameList).map(([key, { name, mark }]) => (
                    ...
                    <div key={file.name}>
                        ...
                        >
                            <img
                                ...
                                alt={file.name}
                                className="cursor-pointer w-32 h-32 object-cover m-2 hover:filter hover:brightness-90"
                                onClick={() => handleDelete(index)}
                            />
                            <button className="BATSU absolute top-0 right-1 pointer-events-none" />
                            <p className="w-32 overflow-hidden text-ellipsis mb-2">
                                {file.name}
                            </p>
                        </div>
                    </div>
